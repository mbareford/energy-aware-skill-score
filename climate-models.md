...
```
where $\bar{x}_{p,\mu}$ is the spatial average of the temporally-averaged ground truth observations.

The expression for $\xi_{i}$ is based on the coefficient of determination. If the averages derived from the model truth are far from the equivalent ground truth, $\xi_{i}$ will be large, unless the ground truth data deviates substantially from the spatial mean. Conversely, $\xi_{i}$ will be small if the model and ground truths match closely.
