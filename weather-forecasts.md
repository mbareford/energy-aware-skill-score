...
Assume a time series of observations for some property distributed over a grid, $x_{i,j}$, where $i$ is the grid cell index covering the range $1-N$ and $j$ is the time index over $1-T$. The corresponding forecasted values are denoted by $f_{i,j}$.

A first approach is to summarise these differences as a root mean square relative error (RMSE), $\sqrt{\frac{\varepsilon_{i}}{T}}$, where

```math
\varepsilon_{i} = \sum_{j=1}^{T} \left( \frac{x_{i,j} - f_{i,j}}{x_{i,j}} \right)^{2} \;.
```

Basing a skill score on such an expression gives rise to two problems however. Firstly, it is possible for $x_{i,j}$ to take zero values (fairly common for measurements of temperature and precipitation) causing $\varepsilon_{i}$ to be undefined. Secondly, converting the RMSE into a skill score by, say, subtracting it from one, can result in a negative score whenever the error is sufficiently large.
...
```math
\omega_{i} = \frac{1}{T-1}\sum_{j=2}^{T} | x_{i,j} - x_{i,j-1} | \;.
```

The $\omega_{i}$ term provides the scaling for the MASE, which we denote by $\xi_{i}$,

```math
\xi_{i} = \frac{\frac{1}{T}\sum_{j=1}^{T} |x_{i,j} - f_{i,j}|}{\omega_{i}} \;.
```
