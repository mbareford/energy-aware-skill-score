...
Next, we explore how to assess the level of achievement implied by the model prediction. We start with a ground truth that can be compared with computer-generated predictions. For a given property, $p$,  such as temperature or salinity, we can calculate the differences between a time series of $N$ observed values ($x_{t}$) and the corresponding forecasted values ($f_{t}$), summarising these as a root mean square relative error, $p / N$, where
